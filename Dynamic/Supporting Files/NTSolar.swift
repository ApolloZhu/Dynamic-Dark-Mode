// https://github.com/ceeK/Solar/issues/40#issuecomment-502465264
//
//    NTSolar.swift
//
...
class NTSolar {
    ...
    class func sunRiseAndSet(forDate: Date,
        ...
        guard let year = comp.year,
            ...
            print("Failed to find date components.")
            return nil
        }

        let (riseUTC, setUTC, code): (Double, Double, ReturnCode)
        switch kind {
        case .official:
            (riseUTC, setUTC, code) = sun_rise_set(
                year: year, month: month, day: day,
                lon: atLocation.longitude, lat: atLocation.latitude
            )
        case .civil:
            (riseUTC, setUTC, code) = civil_twilight(
                year: year, month: month, day: day,
                lon: atLocation.longitude, lat: atLocation.latitude
            )
        case .nautical:
            (riseUTC, setUTC, code) = nautical_twilight(
                year: year, month: month, day: day,
                lon: atLocation.longitude, lat: atLocation.latitude
            )
        case .astronomical:
            (riseUTC, setUTC, code) = astronomical_twilight(
                year: year, month: month, day: day,
                lon: atLocation.longitude, lat: atLocation.latitude
            )
        }
        ...
        if code != .RiseAndSet {
            print("Failed to find rise and set.")
            return nil
        }
